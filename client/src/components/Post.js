...
export default function Get(props) {
    ...
    function handleChange1(evt){
        setSelectedSchool(schools[evt.target.value])
        // console.log(selectedSchool);
    }
    function handleChange2(evt){
        setSelectedData(data[evt.target.value]);
    ...
    function handleChange3(evt){
        ...
        tmp = { promptId: `${responses[evt.target.id-1].promptId}`, choice: `${data2.responses[evt.target.id-1].choices[evt.target.value]}`};
  ...
  useEffect(() => {
    ...
    return (
        <div>
            ...
            <div>
                    <h1>{data2.school}</h1>
                    <h2>{data2.building}</h2>
                    {data2.responses.map((info, index) =>(
                        <div key={`div`+index}>
                            <h3 key={`h3`+index} >{info.name}</h3>

...
export default function Trends(props) {
    ...
    useEffect(() => {
        function getBuildings() {
            axios
                .get(
                    `https://hackgt-unispaces.herokuapp.com/schools/${selectedSchool}/`
                )
    ...
    useEffect(() => {
        function getTrendData() {
            axios
                .get(
                    `https://hackgt-unispaces.herokuapp.com/stats/${selectedSchool}/${selectedBuilding}`
                )
                .then(function (response) {
                    if (response.status === 200) {
                        console.log(response);
            ...
            <div className="get-margins">
                ...
                {data2 !== null &&
                    data2.data.responses.map((info, index) => (
                        <Renderbarchart
                            id={index}
                            response={info.trainedResponses}
                        />
                    ))}
            </div>
        </div>
